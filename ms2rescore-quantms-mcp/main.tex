...
\title{quantms, quantms-rescoring and multiple search engines enables deep proteome coverage across protein quantification, immunopeptidomics, and phosphoproteomics experiments}
...
% Abstract Section
\begin{abstract}
The exponential growth of public proteomics datasets has surpassed the analytical capacity of traditional desktop tools, particularly for large-scale automated reanalysis. To address this challenge, we present an integrated workflow that combines quantms, a cloud-native pipeline, AlphaPeptDeep and with MS²Rescore, a machine learning-based rescoring tool. This workflow enables deep and scalable reanalysis of massive proteomics datasets. Powered by the Nextflow engine for parallel computing, the pipeline incorporates fragment ion intensity predictions from MS²PIP and retention time predictions from DeepLC, improving peptide-spectrum match reliability through Percolator. We applied this approach to four representative datasets covering label-free quantification, TMT labeling, immunopeptidomics, and phosphoproteomics. Compared to traditional methods, our workflow achieved a 16–22.8\% increase in identified spectra, along with the quantification of 2191 additional phosphorylation peptides and 1337 phosphosites. 27 novel 
differentially expressed proteins were identified in a TMT-labeled clear cell renal cell carcinoma dataset by multiple search engines with quantms-rescoring, and are mapped to kidney and renal system development biological process. These improvements demonstrate that integrating multiple search engines with machine learning-derived features not only enhances identification sensitivity but also deepens quantitative insights for downstream biological interpretation. Overall, this workflow offers a reproducible and scalable solution for the reanalysis of public proteomics data, advancing FAIR principles by promoting scientific transparency, accessibility, and data reuse.
...
In recent years, the field of proteomics has experienced rapid growth in the availability of publicly accessible datasets, accompanied by a shift toward studies analyzing larger sample cohorts. As of June 2025, over 40,000 datasets have been submitted to ProteomeXchange (PX) repositories, including a substantial increase in large-scale submissions comprising more than 100 instrument files \cite{perez-riverol_pride_2025}. Although, several popular search engines tools are developed such as MaxQuant \cite{cox_maxquant_2008}, pFind \cite{wang_pfind_2007} and PeptideShaker \cite{vaudel2015peptideshaker}, there is no container with command line interface, or definitely cloud-ready workflow systems like Nextflow and cannot be easily integrated into cloud open-source infrastructures, limiting in their capacity to perform automated, large-scale quantitative analyses in cloud or distributed environments, hindering the reanalysis of extensive experiments on standard workstations. Most of these tools (eg. Proteome Discoverer) are commercial or closed-source, although some have begun adopting cloud-based deployment models.

We recently developed quantms, an open-source, cloud-based pipeline designed for massively parallel reanalysis of quantitative proteomics datasets \cite{dai_quantms_2024} to address computational bottleneck for large-scale quantitative re-analyses and analyses. The pipeline is highly modular and flexible, accommodating a wide range of quantitative proteomics approaches. quantms automatically distributes computations using the Nextflow workflow engine across one or more computing nodes, depending on the number of instrument files and samples \cite{di_tommaso_nextflow_2017}. To ensure traceability and reproducibility, the pipeline is built entirely on standardized open file formats \cite{dai_proteomics_2021} \cite{martens_mzmlcommunity_2011} and reproducible execution environments such as Docker and Singularity, adhering strictly to the FAIR (Findability, Accessibility, Interoperability, and Reusability) principles \cite{wilkinson_fair_2016}. 

With the adoption of machine learning (ML) and deep-learning (DL) in the field of proteomics, various models have been developed to accurately predict peptide behavior in LC-MS, such as MS²PIP \cite{declercq_updated_2023}, AlphaPeptDeep \cite{zeng_alphapeptdeep_2022} and DeepLC \cite{bouwmeester_deeplc_2021} for fragment ion intensities and retention time prediction, respectively. These highly accurate predictions enable superior matching of experimental data to theoretical expectations and have reinvigorated rescoring strategies in proteomics. 
MS²Rescore is a modular Python package that generates multiple features assessing the similarity between observed and predicted peptide behavior, such as fragment ion intensities from MS2PIP, retention time from DeepLC, and ion mobility \cite{buur_ms2_2024} \cite{declercq_tims2_2025}.

Previously, quantms did not leverage measurable peptide properties such as fragment ion intensities and retention times, and MS²Rescore still relies on a single machine learning-based peptide behavior predictor for some feature, such as fragment ion intensities. To overcome this limitation and further enhance performance and applicability, we developed quantms-rescoring python package that integrated MS²PIP, AlphaPeptDeep, DeepLC and MS²Rescore into quantms multiple search engines workflow and  incorporated user customized features such as signal-to-noise ratio (SNR) following Nextflow and nf-core best practices \cite{ewels_nf-core_2020}. quantms-rescoring supports selecting the optimal model from all candidate models.
...
% HW: Is it shown for all datasets?
